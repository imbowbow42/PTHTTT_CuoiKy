...
def index():
    ...
    # Close Connection
    cur.close()
    return render_template('home.html', laptop=laptop, screen=screen, keyboard=keyboard, mouse=mouse, form=form)
    ...
    if 'view' in request.args:
        ...
        if 'uid' in session:
            ...
            if result:
                ...
                mysql.connection.commit()
        return render_template('view_product.html', x=x, laptops=product)
        ...
        product = curso.fetchall()
        x = content_based_filtering(product_id)
        return render_template('order_product.html', x=x, laptops=product, form=form)
    return render_template('laptop.html', laptop=products, form=form)
...
def screen():
    ...
    if 'view' in request.args:
        ...
        return render_template('view_product.html', x=x, laptops=products)
    elif 'order' in request.args:
        product_id = request.args['order']
        curso = mysql.connection.cursor()
        curso.execute("SELECT * FROM products WHERE id=%s", (product_id,))
